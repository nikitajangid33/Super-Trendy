...
$getposts = mysqli_query($ccon,"SELECT * FROM products WHERE id ='$epid'") or die(mysqli_error());
...
if (isset($_POST['updatepro'])) {
	...
	if($result = mysqli_query($ccon,"UPDATE products SET pName='$_POST[pname]',price='$_POST[price]',description='$_POST[descri]',available='$_POST[available]',category='$_POST[category]',type='$_POST[type]',item='$_POST[item]',pCode='$_POST[code]' WHERE id='$epid'")){
...
if (((@$_FILES['profilepic']['type']=='image/jpeg') || (@$_FILES['profilepic']['type']=='image/png') || (@$_FILES['profilepic']['type']=='image/jpg') || (@$_FILES['profilepic']['type']=='image/gif')) && (@$_FILES['profilepic']['size'] < 1000000)) {
	...
	if (file_exists("../image/product/$item/".$filename)) {
		...
		if(move_uploaded_file(@$_FILES["profilepic"]["tmp_name"], "../image/product/$item/".$filename)){
			$photos = $filename;
			if($result = mysqli_query($ccon,"UPDATE products SET picture='$photos' WHERE id='$epid'")){
...
//triming name
	$getposts1 = mysqli_query("SELECT pid FROM orders WHERE pid='$epid'") or die(mysqli_error());
					if ($ttl = mysqli_num_rows($getposts1)) {
						$error_message = "You can not delete this product.<br>Someone ordered this.";
					}
					else {
						if(mysqli_query("DELETE FROM products WHERE id='$epid'")){
