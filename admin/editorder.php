...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$eoid = $_REQUEST['eoid'];
		$getposts5 = mysqli_query($ccon,"SELECT * FROM orders WHERE id='$eoid'") or die(mysqli_error());
	...
	$result1 = mysqli_query($ccon,"SELECT * FROM orders WHERE id='$eoid'");
		$get_order_info = mysqli_fetch_assoc($result1);
			...
			$result2 = mysqli_query($ccon,"SELECT * FROM user WHERE id='$eouid'");
...
$getposts = mysqli_query($ccon,"SELECT * FROM products WHERE id ='$eopid'") or die(mysqli_error());
...
//triming name
	if(mysqli_query($ccon,"DELETE FROM orders WHERE id='$eoid'")){
