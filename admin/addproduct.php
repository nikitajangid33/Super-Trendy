...
if (((@$_FILES['profilepic']['type']=='image/jpeg') || (@$_FILES['profilepic']['type']=='image/png') || (@$_FILES['profilepic']['type']=='image/gif')) && (@$_FILES['profilepic']['size'] < 1000000)) {
	...
	if (file_exists("../image/product/$item/".$filename)) {
		...
		if(move_uploaded_file(@$_FILES["profilepic"]["tmp_name"], "../image/product/$item/".$filename)){
			$photos = $filename;
			$result = mysqli_query($ccon,"INSERT INTO products(pName,price,piece,description,available,category,type,item,pCode,picture) VALUES ('$_POST[pname]','$_POST[price]','$_POST[piece]','$_POST[descri]','$_POST[available]','$_POST[category]','$_POST[type]','$_POST[item]','$_POST[code]','$photos')");
  											...
  											<h5>Item:</h5>
												...
												<select name="item" required="required" style=" font-size: 20px;
														font-style: italic;margin-bottom: 3px;margin-top: 0px;padding: 14px;line-height: 25px;border-radius: 4px;border: 1px solid #169E8F;color: #169E8F;margin-left: 195px;width: 300px;background-color: transparent;" class="">
																<option selected value="menwomen">Men & Women Wear</option>
																<option value="partywear">Party Wear</option>
																<option value="kidfashion">Kids Wear</option>
																<option value="footwear">Foot wear</option>
																<option value="sweet">Accessories</option>
																<option value="snack">Watches</option>
																<option value="shampoo">Drinks</option>
