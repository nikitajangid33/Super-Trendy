...
if (isset($_POST['login'])) {
	...
	//	$user_login = mysqli_real_escape_string($_POST['email']);
		$user_login = mb_convert_case($_POST['email'], MB_CASE_LOWER, "UTF-8");
		$password_login = $_POST['password'];
		$num = 0;
		$password_login_md5 = md5($password_login);
		$result = mysqli_query($ccon,"SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='yes'");
		...
		if ($num>0) {
			...
			$result1 = mysqli_query($ccon,"SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='no'");
...
if(isset($_POST['activate'])){
	if(isset($_POST['actcode'])){
		...
		$result2 = mysqli_query($ccon,"SELECT * FROM user WHERE (email='$user_login') AND confirmCode='$user_acccode'");
		...
		if ($num3>0) {
			...
			$_SESSION['user_login'] = $get_user_uname_db;
			setcookie('user_login', $user_login, time() + (365 * 24 * 60 * 60), "/");
			mysqli_query($ccon,"UPDATE user SET confirmCode='0', activation='yes' WHERE email='$user_login'");
			if (isset($_REQUEST['ono'])) {
				$ono = $_REQUEST['ono'];
				header("location: orderform.php?poid=".$ono."");
			}else {
